...
fn test_delegate_at_increase_success() {
    ...
    let checkpoint_type_script = context
        .build_script(&always_success_out_point, Bytes::from(vec![2u8; 32]))
    ...
    let delegate_args = delegate::DelegateArgs::new_builder()
        ...
        .delegator_addr(axon_identity_opt(&delegator_keypair.1.serialize()))
        .build();
...
fn test_delegate_smt_success() {
    ...
    let delegate_at_args = delegate::DelegateArgs::new_builder()
        ...
        .build();
    let delegate_at_lock_script = context
        .build_script(&contract_out_point, delegate_at_args.as_bytes())
    ...
    let delegate_smt_type_script = context
        .build_script(&contract_out_point, delegate_smt_args.as_bytes())
        .expect("delegate smt type script");
    let delegate_infos = BTreeSet::new();
    let (input_delegate_smt_cell_data, input_delegate_epoch_proof) =
        axon_delegate_smt_cell_data(&delegate_infos, &staker_keypair.1);
    ...
    let (output_delegate_smt_cell_data, output_delegate_epoch_proof) =
        axon_delegate_smt_cell_data(&output_delegate_infos, &staker_keypair.1);
